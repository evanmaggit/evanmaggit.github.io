async function loadClownPage() {

  try {

    const response = await fetch("clown.json");

    if (!response.ok) {

      throw new Error("Could not load clown.json");
    ...
    document.getElementById("film-logline").textContent =
      ...
      "Film details could not be loaded. Please check that clown.json is in the same folder as clown.html.";
