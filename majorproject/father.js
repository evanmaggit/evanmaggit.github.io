async function loadFatherPage() {
  try {
    const response = await fetch("father.json");

    if (!response.ok) {
      throw new Error("Could not load father.json");
    }
    ...
    document.getElementById("film-logline").textContent =
      "Film details could not be loaded. Please check that father.json is in the same folder as father.html.";
  }
}
